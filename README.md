...
Большинство значений 'terminal_id' находятся в диапазоне {0, 999}. Однако есть случаи, которые отличаются: NaN - 13060; Err - 27252; 89518096 - 513023; 21861 - 59066; 618230 - 2643; 22225 - 333; 436002 - 90; 22129 - 1; 360740 - 1. Эти наблюдения удалены, из них решено не делать отдельных категорий, так как эти категории показали отличающаеся поведение по целевой переменной от других. Вероятно такие транзакции можно блокировать, так как данные по ним скорее всего некорректные.

### 6. Некорректные значения 'customer_id': -999999
Обнаружено 14623 транзакций со значением 'customer_id': -999999. Их решено удалить, так как статистика по этим транзакциям отлична от других значений, если оценивать в разрезе 'terminal_id'. Также вероятно для модели потребуется собирать аггрегаты по клиентам, а аггрегат по данному значению может быть неинформативным. Транзакции, в которых 'customer_id': -999999 также вероятно можно блокировать из-за некорректных данных.
